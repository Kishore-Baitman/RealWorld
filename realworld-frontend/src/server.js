...
// Add a test user for the profile page
const testUser = {
  email: 'baitman@example.com',
  username: 'baitman',
  bio: 'This is a test bio for the baitman user',
  ...
  return createServer({
    ...
    seeds(server) {
      ...
      if (db) {
        ...
        // Add the test user
        const baitman = server.create('user', testUser)
        sampleSize(users, 3).forEach((user) => {
          user.update({ following: true })
        })
        const allUsers = [...users, user, baitman]
    ...
    routes() {
      ...
      this.passthrough((request) => {
        return request.url.startsWith('http://localhost:5173')
      })
      ...
      this.post('/users/login', (schema, request) => {
        ...
        const foundUser = schema.users.findBy({ email: user.email })
        
        if (!foundUser) {
          return new Response(422, {}, { errors: { 'email or password': ['is invalid'] } })
        }
        
        // Return the user object as is, since it already has a token from the factory
      ...
      this.post('/users', (schema, request) => {
        ...
        const existingUser = schema.users.findBy({ email: user.email })
        if (existingUser) {
          return new Response(422, {}, { errors: { email: ['has already been taken'] } })
        }

        // Create a new user - the factory will automatically add a token
      ...
      this.put('/user', (schema, request) => {
        ...
        const updatedUser = schema.users.findBy({ email: authUser.email }).update(user)
      ...
      this.get('/profiles/:username', (schema, request) => {
        ...
        if (!user) {
          return new Response(404, {}, { error: 'Profile not found' })
        }
